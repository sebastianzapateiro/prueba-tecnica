...
function DescriptionPokemonF() {
  ...
  return (
    ...
    <Container className='mt-5'>
      ...
      <Row>
        <Col>
          <img src={datapokemonF[0]?.image} alt="Bulbasaur" />
        </Col>
        <Col>
          <div className='d-flex flex-column'>
            ...
            <div><span>Tipos: </span>
              {datapokemonF[0]?.tipo.map((tipo, llave) => (
                <span key={llave} >{tipo}&nbsp;</span>
              ))
              }
            </div>
            <div><span>Altura: </span><span>{datapokemonF[0]?.altura}</span></div>
            <div><span>Peso: </span><span>{datapokemonF[0]?.peso}</span></div>
            <div><span>Abilidades: </span>{datapokemonF[0]?.abilities.map((tipo, llave) => (
              <span key={llave} >{tipo}&nbsp;</span>
            ))
            }</div>
            <div><span>Grupo de huevos: </span>
            {datapokemonF[0]?.egg_groups?.map((tipo, llave) => (
                <span key={llave} >{tipo}&nbsp;</span>
              ))
              }
            <span>
